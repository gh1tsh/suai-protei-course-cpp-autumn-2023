				    ...
				    unsigned zone_id) const
{
    ...
    // NOTE: you can remove the result variable and use the result of the
    //       expression immediately in return
					                          ...
					                          unsigned int zid)
{
	...
	auto iter_to_target_zone = std::find(
                                        zones_list.begin(),
                                        zones_list.end(),
                                        zones.at(zid));
	...
	auto iter_to_target_subscriber = std::find(
                                            subscribers_list.begin(),
...
void Locator::load_subscriber_data_from_file(std::string const& path_to_file)
{
	...
	try {
        // FIXME: hardcoded path to subscriber's data schema file
		schema_file.open("/home/ghitsh/suai/suai-protei-course-cpp/stages/stage-2/locator-project/json-schemas/subscriber_data_schema.json");
	} catch (std::exception& e) {
		spdlog::error(
...
void Locator::load_locator_config()
{
	std::ifstream schema_file;
	try {
        // FIXME: hardcoded path to schema
		schema_file.open("/home/ghitsh/suai/suai-protei-course-cpp/stages/stage-2/locator-project/json-schemas/locator_config_schema.json");
		// schema_file.open("../../json-schemas/locator_config_schema.json");
	} catch (std::exception& e) {
		spdlog::error(
            // FIXME: rewrite error messages to something like:
            //        "Can not parse..."
	...
	try {
        // FIXME: hardcoded locator config path
		locator_config_json_file.open("/home/ghitsh/suai/suai-protei-course-cpp/stages/stage-2/locator-project/locator-config/locator_config.json");
	} catch (std::exception& e) {
		spdlog::error(
